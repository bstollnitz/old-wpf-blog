...
namespace CustomControls
{
    ...
    /// </summary>
    [TemplatePart(Name = "Panel_PART", Type = typeof(Panel))]
    ...
    public class PieChartLabel : ContentControl
    {
        ...
        public override void OnApplyTemplate()
        {
            ...
            contentPart = this.GetTemplateChild("Content_PART") as FrameworkElement;
            panelPart = this.GetTemplateChild("Panel_PART") as Panel;
            polylinePart = this.GetTemplateChild("Polyline_PART") as Polyline;
        }
        ...
        public Point PositionLabel()
        {
            switch (this.DisplayMode)
            {
                case DisplayMode.ArcMidpoint:
                    return this.PositionArcMidpoint();
                case DisplayMode.Connected:
                    return this.PositionConnected();
                case DisplayMode.AutoMixed:
                    return this.PositionAutoMixed();
                case DisplayMode.Auto:
                    return this.PositionAuto();
            }
            return new Point();
        }
        ...
        /// </summary>
        /// <returns>Return the top and left position of the label in the label area panel.</returns>
        private Point PositionArcMidpoint()
        {
            //this.RemovePolyline();
            if (this.polylinePart != null)
            {
                this.polylinePart.Visibility = Visibility.Collapsed;
            }

            if (this.contentPart != null)
            {
                //this.contentPart.SetValue(Canvas.TopProperty, this.arcMidpoint.Y - 0.5 * this.contentPart.ActualHeight);
                //this.contentPart.SetValue(Canvas.LeftProperty, this.arcMidpoint.X - 0.5 * this.contentPart.ActualWidth);
                return new Point(this.arcMidpoint.X - 0.5 * this.contentPart.ActualWidth, this.arcMidpoint.Y - 0.5 * this.contentPart.ActualHeight);
            }
            return new Point();
        }
        ...
        /// </summary>
        /// <returns>Return the top and left position of the label in the label area panel.</returns>
        private Point PositionConnected()
        {
            //RemovePolyline();
            if (this.polylinePart != null)
            {
                this.polylinePart.Visibility = Visibility.Visible;
            }
            ...
            if (this.contentPart != null)
            {
                ...
                //this.contentPart.SetValue(Canvas.TopProperty, thirdPoint.Y - 0.5 * this.contentPart.ActualHeight);
                double contentX = (sign == 1) ? thirdPoint.X : thirdPoint.X - this.contentPart.ActualWidth;
                double contentY = thirdPoint.Y - 0.5 * this.contentPart.ActualHeight;
                //this.contentPart.SetValue(Canvas.LeftProperty, contentX);
                ...
                //this.panelPart.Children.Add(polyline);

                Canvas.SetLeft(polylinePart, -contentX);
                Canvas.SetTop(polylinePart, -contentY);
                polylinePart.Points = newPoints;
        ...
        private void RemovePolyline()
        {
            ...
            {
                Polyline polyline = panelPart.Children.OfType<Polyline>().FirstOrDefault();
                if (polyline != null)
                {
                    panelPart.Children.Remove(polyline);
                }
            }
        }
        ...
        /// </summary>
        /// <returns>Return the top and left position of the label in the label area panel.</returns>
        private Point PositionAutoMixed()
        {
            if (this.IsArcSmall)
            {
                return this.PositionConnected();
